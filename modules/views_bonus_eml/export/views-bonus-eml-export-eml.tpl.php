...
foreach ($all_vocabularies as $voc) {
  if ($voc->name == 'LTER Keywords' || $voc->name == 'NTL Keywords' || $voc->name == 'Keywords') {
...
if (!empty($keyword_vids)) {
  ...
  foreach ($keyword_vids as $vid) {
    ...
    $keyword_terms = array_merge($keyword_terms, $voc_terms);
  }
  ...
  if(!empty($keywords)) {
    ...
    if (strpos($_SERVER['SERVER_NAME'], 'wisc.edu') !== false) {
      eml_print_line('keywordThesaurus', 'NTL LTER Thesaurus');
    } else {
    // XXX: each site must customize the Thesaurus label somehow; providing a default 'LTER' for now
      eml_print_line('keywordThesaurus', 'LTER Thesaurus');
    }
